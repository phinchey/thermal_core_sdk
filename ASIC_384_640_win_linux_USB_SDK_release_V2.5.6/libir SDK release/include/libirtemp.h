...
typedef struct {
	...
	float    ems;				///< The target emissivity(0-1)
	float    hum;				///< The environment humidity(0-1)
	float    ta;				///< The atmospheric temperature(celcius temperature, unit:¡æ)
	float    tu;				///< The reflection temperature(celcius temperature, unit:¡æ)
}EnvCorrectParam;
...
/**
* @brief new temp correct method in MINI640/384 compatible old and new correct table
*
...
* @param[in] org_temp The origin temperature of target(celcius temperature, unit:¡æ)
* @param[out] new_temp The new temperature after correct(celcius temperature, unit:¡æ)
*
* @return see irtemp_error_t
*/
DLLEXPORT irtemp_error_t enhance_distance_temp_correct(EnvCorrectParam* env_correct_param, uint16_t* correct_table, uint32_t table_len, \
